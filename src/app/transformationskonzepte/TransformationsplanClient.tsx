...
import { transfContent } from "./content";
import ContactCTA from "../components/ContactCTA";
import FaqSection from "../components/FaqSection";
import RelatedSolutions from "../components/RelatedSolutions";

const RELATED_HREFS = ["/foerderung", "/co2-bilanzierung", "/dekarbonisierung"];

export default function TransformationsplanClient() {
  const { language } = useLanguage();
  const t = translations[language] || translations.de;
  const c = transfContent[language] || transfContent.de;
  return (
    <div className="min-h-screen flex flex-col font-sans bg-white text-gray-900">
      ...
      <main className="flex-1 px-4 py-8 max-w-5xl mx-auto w-full">
        ...
        {/* Begriffshinweis: BAFA nennt das Programm inzwischen Transformationsplan,
            der aeltere Begriff Transformationskonzept bleibt im Text erhalten,
            weil Nutzer weiterhin danach suchen. */}
        <section className="w-full pb-8">
          <div className="max-w-3xl mx-auto px-4">
            <p className="text-sm leading-relaxed text-center" style={{ color: "#6B6B6B" }}>
              {c.begriffHinweis}
            </p>
          </div>
        </section>
      ...
      <FaqSection items={c.faq} heading={c.faqTitle} />

      <RelatedSolutions
        heading={c.relatedTitle}
        items={c.related.map((item, i) => ({ ...item, href: RELATED_HREFS[i] }))}
      />

      <ContactCTA location="transformationsplan" context={c.ctaContext} />

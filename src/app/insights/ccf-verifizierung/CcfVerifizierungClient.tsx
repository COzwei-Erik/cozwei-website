...
import { StatCallout, PullQuote, PatternBox } from "../../components/InsightCallouts";
import { OUTLOOK_BOOKING_URL } from "../../links";
import { trackEvent, Events } from "../../analytics";
import { content } from "./content";

// Eriks Outlook-Kalender für 30-Min-Erstgespräche zur CCF-Verifizierung.
const ERIK_BOOKING_URL = OUTLOOK_BOOKING_URL;

export default function CcfVerifizierungClient() {
    ...
    <div className="min-h-screen flex flex-col font-sans bg-white text-gray-900">
      ...
      <main className="flex-1 px-4 py-12 max-w-3xl mx-auto w-full">
        ...
        <article className="prose-article">
          ...
          <p className="text-lg leading-relaxed mb-6" style={{ color: "#23243a" }}>{c.section2P}</p>
          <ul className="list-disc list-outside ml-6 space-y-3 text-lg mb-6" style={{ color: "#23243a" }}>
            {c.section2Bullets.map((b, i) => (
              <li key={i}><strong>{b.strong}</strong>{b.rest}</li>
            ))}
          </ul>

          <PullQuote author={c.pullQuoteAuthor}>{c.pullQuote}</PullQuote>
          ...
          <p className="text-lg leading-relaxed mb-6" style={{ color: "#23243a" }}>{c.section4P}</p>
          <ol className="list-decimal list-outside ml-6 space-y-3 text-lg mb-6" style={{ color: "#23243a" }}>
            {c.section4Items.map((b, i) => (
              <li key={i}><strong>{b.strong}</strong>{b.rest}</li>
            ))}
          </ol>

          <h2 className="text-2xl sm:text-3xl font-extrabold mt-12 mb-4" style={{ color: "#3D405B" }}>{c.section5Title}</h2>
          <p className="text-lg leading-relaxed mb-6" style={{ color: "#23243a" }}>{c.section5P}</p>
          {c.patterns.map((p, i) => (
            <PatternBox key={i} number={i + 1} title={p.title}>{p.body}</PatternBox>
          ))}

          <h2 className="text-2xl sm:text-3xl font-extrabold mt-12 mb-4" style={{ color: "#3D405B" }}>{c.section6Title}</h2>
          <p className="text-lg leading-relaxed mb-6" style={{ color: "#23243a" }}>{c.section6P1}</p>
          <p className="text-lg leading-relaxed mb-6" style={{ color: "#23243a" }}>{c.section6P2}</p>
          ...
          {/* Ihr Ansprechpartner — Erik Jakob (ersetzt die frühere CtaBox an dieser Stelle und die alte ContactForm am Seitenende) */}
            ...
            <div className="rounded-3xl border border-[#81B29A]/30 bg-[#81B29A]/5 p-6 sm:p-8 flex flex-col md:flex-row items-start gap-6">
              <div className="relative w-28 h-28 sm:w-36 sm:h-36 flex-shrink-0 rounded-full overflow-hidden border-2 border-[#81B29A]/30 mx-auto md:mx-0">
                <Image
                  ...
                  alt={`Erik Jakob, ${c.authorRole} bei COzwei`}
                  fill
                  sizes="144px"
              ...
              <div className="flex-1">
                <h3 className="text-xl sm:text-2xl font-extrabold mb-1" style={{ color: "#3D405B" }}>Erik Jakob</h3>
                ...
                <a
                  href={ERIK_BOOKING_URL}
                  target="_blank"
                  rel="noopener noreferrer"
                  onClick={() => trackEvent(Events.BookingClick, { location: "ccf_author" })}
